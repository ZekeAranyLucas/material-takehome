...
public class ImfsTreeFileSystem extends ImfsFileSystem {
    ...
    public ImfsTreeFileSystem(ImfsProvider imfsProvider, String key) {
    ...
    public ImfsDirectoryStream streamAllPaths(Filter<? super Path> filter) {
        return new ImfsDirectoryStream(this, key, "", records.keySet().stream(), records.size(), filter);
    }
    ...
    public ImfsDirectoryStream streamChildren(String materializedPath, Filter<? super Path> filter) {
        ...
        return new ImfsDirectoryStream(this, key, materializedPath, sub.keySet().stream(), sub.size(), filter);
    ...
    public void putBlob(String materializedPath, byte[] bytes) {
        records.put(materializedPath,
                ImfsRecord.builder().materializedPath(materializedPath).file(true).bytes(bytes).build());
    }
